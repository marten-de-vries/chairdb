...
def test_new_branch():
    ...
    tree.merge_with_path(doc_rev_num=3, doc_path=['h', 'c'], doc_ptr={})
...
def test_order():
    ...
    tree.merge_with_path(doc_rev_num=1, doc_path=['b'], doc_ptr={'x': 1})
    validate_rev_tree(tree)
    ...
    tree.merge_with_path(doc_rev_num=1, doc_path=['a'], doc_ptr={'x': 2})
    validate_rev_tree(tree)
    ...
    tree.merge_with_path(doc_rev_num=1, doc_path=['c'], doc_ptr={'x': 3})
    validate_rev_tree(tree)
...
def test_revs_limit_basic():
    ...
    tree.merge_with_path(doc_rev_num=2, doc_path=['b', 'a'], doc_ptr={},
                         revs_limit=1)
    validate_rev_tree(tree)
...
def test_revs_limit_advanced():
    ...
    #
    # 2-b 3-c 4-d
    tree.merge_with_path(doc_rev_num=4, doc_path=['d', 'c', 'b', 'a'],
