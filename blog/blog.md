...
subtitle: "[TXT](blog.md) | [EPUB](blog.epub) | [PDF](blog.pdf)"
...
Forwarding to today, CouchDB is still going strong, and currently in the process of [having its internals rewritten](https://blog.couchdb.org/2020/02/26/the-road-to-couchdb-3-0-prepare-for-4-0/) on top of [FoundationDB](https://www.foundationdb.org/). There's even ongoing work [to support document-level access control](https://github.com/apache/couchdb-documentation/pull/424/files). PouchDB has matured, and is a popular IndexedDB wrapper. Having replication functionality is the feature that sets it apart among those.

## Goal

My aim so far has been to explain why I think CouchDB and its replication are interesting, and to show the state of the field. While I have been a long-time CouchDB user, I've never gotten around to actually trying to understand how it and its replication work **from first principles**. When I recently decided to do so my first thought was to look at PouchDB's internals[^internals], but that alone was not enough. PouchDB is a mature implementation which handles a lot of edge cases, and was written at a time when callbacks were still the only way of doing asynchronous control flow in JavaScript. To get the overview I decided to build my own minimal prototype of a CouchDB-compatible database including replication instead. This blog post summarizes my attempt to do so using the [Python programming language](https://www.python.org/).

Before we get to it, I should mention [Alexander Shorin previously embarked on a similar project](https://github.com/kxepal/replipy). In my opinion it oversimplifies conflict handling a bit, which comes at a cost to correctness, but it's an impressive resource worth checking out nonetheless. More recently, [Garren Smith wrote a mini-CouchDB in Rust](https://www.garrensmith.com/blogs/mini-couch-hack-week). That prototype isn't complete enough to support replication, but it gives a nice introduction to using FoundationDB like CouchDB will soon do. Very much worth a look as well!

[^internals]: <https://github.com/pouchdb/pouchdb>. CouchDB's [source code](https://github.com/apache/couchdb-couch) is pretty accessable as well, but it's written in Erlang, which I can not write and only somewhat read.
...
Imagine a municipality that keeps track of all the trees it plants in a database. In the morning, Bob comes along and plants a tree somewhere at a roadside. He dutifully updates the record for the current plot of land on his smartphone (revision '``1-1a9c``' gets replaced with '``2-e3b0``'), but as this is in a remote spot, the information is not synced to the municipality's server immediately. To make matters worse, his phone's battery dies before this can happen. While the phone is switched off, Jane reaches the same location with a second tree. She plants it, updates the database record (revision '``1-1a9c``' gets replaced with '``2-6e05``'), and goes on her way. Soon afterward, her phone syncs her changes to the server (which now has '``2-6e05``' as its latest revision).
...
Clearly, this is unacceptable. If nothing were done, Jane's record update would be lost, through no fault of her own. This is why CouchDB's documentation recommends you to either resolve conflicts on each read, or to have a background process that watches for conflicts and resolves them. Let's say the municipality uses the latter approach. This background process then requests the two latest versions of the document. It creates a record which includes both of the newly planted trees, and replaces '``2-e3b0``' (Bob's revision) with a new revision '``3-5bd6``'. This doesn't yet resolve the conflict, for that, we also need to remove Jane's revision ('``2-6e05``'). We do so by replacing it with a new revision ('``3-b617``') which marks the removal. This revision is also called a '**tombstone revision**' in CouchDB terminology, and is often represented by an (otherwise empty) document with a '``_deleted``' field.

Note that if the background process had removed Bob's revision ('``2-e3b0``') and stored the new revision as a continuation of Jane's revision ('``2-6e05``') instead, this also would have resolved the conflict. The important point is that there should be at most one last revision that is not a tombstone revision. The above also means that if you remove a document that has conflicts, it will not disappear. Instead, the conflict with the highest revision will be promoted to be the new 'winner'.
...
Instead, CouchDB-like databases keep all previous revisions. The most efficient way to store this is in a tree structure called the '**revision tree**'. The revision tree for our municipality example was already given above as a figure. Our example tree contains two **leaf nodes**, one of which is a tombstone revision, and has a single **root node**. To make sure revision trees don't increase in size infinitely as data is added to the database, CouchDB-like databases implement two mechanisms:

1. **Compaction**. This removes all information about non-leaf nodes except their revision. In our example, that means that no information about the number of trees that are planted is kept except for the final record ('42 trees planted'). Compaction is the reason you should not rely on revisions as a history mechanism in CouchDB. If you require history information, you need to store it explicitly during each document update instead. To simplify our implementation, we design our data structures such that they can only store document information about the leaf node. This is equivalent to performing compaction immediately after each write.
2. **Revision pruning**. This throws away revisions that are old. 'Old' in this case means that a certain number of revisions (**the revisions limit**) were added between the current node and any of its leaf nodes.

## Representation

With all that out of the way, it's time to get busy. Let's implement a revision tree first. The easiest way is to implement it as a list of branches[^whynotree]. It makes the revision tree for the roadside record look like this in code:

[^whynotree]: Why a list of branches and not a real tree structure? I'm glad you asked. The short answer is that it makes a lot of the algorithms described here more complex. It was my initial approach, but I never got revision pruning to work that way (see TODO: bd35b4212e4acffa3a0442b273c2ea3ba1280060). It's possible to do it I assume, but it's hard, to the point where CouchDB and PouchDB both convert their internal tree structure into the same 'list of branches' representation used here and back again when doing revision pruning. A tree would definitely waste less memory, though.
...
It does. Now what about ``RevisionTree``? As we saw, our ``RevisionTree`` representation is essentially a ``list``, so let's inherit from that. If we keep the branches ordered by highest revision number and hash, that should make determining the winning revision easier later on.
...
So far so good. Now let's implement some of the revision tree operations we discussed. First, finding the winning revision:
...
Note that the loop first iterates over the branch with the highest leaf revision, and only then considers the others (also in order). Let's make sure this correctly points to Bob's branch as the winner:
...
Useful. But what if we don't want to get just the winner, but want to find branches by revision? That's doable too:
...
```python
>>> tree.find(2, 'e3b0') == bob_branch
True
```

Success! One final reading method. What if we want to enumerate all revisions in the tree? That might not sound useful now, but I promise we'll find a use for it when implementing replication later on:
...
Alright, so by now we have a feel of what a revision tree looks like, but we need to clarify how to build it from scratch. What information is available during insertion? Well, definitely the new revision's revision number. We also talked about requiring one or more ancestor revision hashes next to the 'new' one. And of course we need the document contents. Oh, and finally this revision limit. Now, it turns out there are four possible cases during insertion. But our first function only handles two of them:

1. The revision that we try to insert is already in the tree. This case is easy, we simply do nothing.
2. The typical case that Jane and Bob experienced. They extended a previous revision ('``1-1a9c``') that was a leaf revision with one (or more) new revision(s): '``2-e3b0``' and '``2-6e05``', in their case.

We implement that as follows. There are inline examples to understand how the
code checks for these cases:
...
Note that in the typical case (2), we no longer need the 'old' branch, as it is completely incorporated into the 'extended' branch. That's the reason the ``del``ete statements are in there. There are some unfamiliar functions in this code block. We'll get to ``self._insert_as_new_branch`` soon, it implements the third and
fourth cases I talked about previously. But this is a good moment to introduce ``self._insert_branch``,
...
As you can see, applying revision pruning is extremely simple with this implementation. If you don't know about [bisect.bisect](https://docs.python.org/3/library/bisect.html), here's the short version: given a sorted list, it gives you the place to insert an item which will keep the list sorted. And it can do so efficiently without going through the whole list[^timecomplexity].

[^timecomplexity]: In [$O(\log n)$](https://en.wikipedia.org/wiki/Big_O_notation), to be exact. But if you know what that means, I probably don't have to explain binary search in the first place.
...
4. It's possible that no common ancestors can be found. This probably occurs most commonly when the ``RevisionTree`` is still completely empty. We simply insert the new branch as-is.

The code to handle these two cases is as follows, again with in-line examples:
...
That's all there is to the RevisionTree. Let's try out our nice new merge method:
...
As this post tries to build a minimal database, we won't worry about persisting data to disk. Instead, we'll build an in-memory database. For indexes, we use [sortedcontainers](http://www.grantjenks.com/docs/sortedcontainers/)'s [SortedDict](http://www.grantjenks.com/docs/sortedcontainers/sorteddict.html) type. This allows us to both efficiently iterate over keys in sorted order[^complexity1], and to quickly retrieve their associated values[^complexity2].

On the other hand, we will make an effort to separate 'in memory'-database logic from other logic, so we can easily build disk-backed databases later.

Time to introduce some CouchDB concepts just in case you haven't heard of them:

- **local documents**. The normal rules don't apply to these documents. They cannot be replicated and do not keep a revision tree. Every write overwrites whatever was stored under their '``_id``' previously. Because of this, we implement a separate index for them. They will also have their own specialized (but simpler) code.
- **update sequence**. This keeps track of the amount of writes that occured in the database.
...
You see we have a the separate index for local documents, as promised. That leaves the other two. First, the '**by seq**' index maps the update sequence at the time of the last update of a document to that document's id. This allows us to reconstruct the order in which all documents were updated. Replication needs that to keep track of what changed. But perhaps the most important index is the '**by id**' index[^indices]. It maps document IDs to a ``DocumentInfo`` object. Which is another simple named tuple type:
...
You already know about the revision tree. We cache the output of ``RevTree.winner_idx()`` after each write in the ``winning_branch_idx`` field for efficiency. Finally, ``last_update_seq`` is the same value that's used as key in the 'by sequence' index. We keep it around so we can remove old records from that index.

[^indices]: Curious how other implementations handle indices? I know I was. The on-disk file format of [couchstore can be found here](https://github.com/couchbaselabs/couchstore/wiki/Format). A discussion of PouchDB's latest database schema [can be found here](https://github.com/couchbaselabs/couchstore/wiki/Format).
...
2. extract the revision number, revision hash and ancestor revision hashes (and remove them from the document)
  - note: a local document doesn't have any of those.
...
This code path just updates the key value store:
...
There's quite a bit going on here. Let's break it down. At this point, it's clear we're going to modify the database. We'll have to update both of the main indices. Our first step is to get the existing revision tree (if there is one) for this document ID. Soon, we'll overwrite it with a new one. We also remove the soon-to-be outdated entry in the 'by sequence' index.

After that's done, we update the revision tree to include the new revision. We'll get to the details of that soon. The important thing is that we get back a fresh revision tree and winning branch index from this process. The only thing left to do for us is to increment the 'update sequence', as the database is being modified, and to store the new values in the indices.
...
It's not that complicated: we simply create a new revision tree if there wasn't one, and merge the user's new revision into the existing tree. Then we return the values we need in ``_write_normal``. All the real logic was already implemented previously.

And that's all there is to writing a document[^couchdb]. Let's try it out, playing out what happens on Jane's phone pre-syncing:
...
So far so good. But that last line isn't user-friendly at all. Let's implement a better API.
...
Probably the most common operation on a database is to read a document given the document ID. But as we've seen, there isn't necessarily a single version. Instead, we allow the user to specify which version(s) they want to retrieve using the ``revs`` argument. It can have a couple of different values:

- ``'winner'``: in this case, we just return the winning revision's document. Use with caution, because this can give unexpected results if there are conflicts. It's [what you get by default](https://docs.couchdb.org/en/stable/api/document/common.html#get--db-docid) in CouchDB if you request a document without any further information.
- ``'all'``: in this case, we return all leaf revision documents. This includes (trivial) documents in the case of tombstone revisions. The CouchDB equivalent is [setting open_revs=all](https://docs.couchdb.org/en/stable/api/document/common.html#get--db-docid).
- a list of revisions. This is also like CouchDB's open_revs option. In this case, we return a document for each of the requested revisions. To be exact, we return the leaf document of the branch in which the revision occurs. Remember, we do not actually keep the document of old revisions around, like CouchDB does (for a while). Instead, it's like we're always using [CouchDB's latest=true option](https://docs.couchdb.org/en/stable/api/document/common.html#get--db-docid).

There is one final parameter to our read method. It's called ``include_path`` and stores whether ancestor revisions should be included in a special [``_revisions`` field](https://docs.couchdb.org/en/stable/api/document/common.html#getting-a-list-of-revisions) of the returned document. This is useful during replication.

Phew. Time to look at the code:
...
We'll ignore the other cases for now, and look at ``to_doc`` first. It
...
Next, let's look at the cases where ``revs`` is not 'winner'. First, the 'all' case, which is easy thanks to our trusty ``RevisionTree.branches()``. And actually, the other case isn't much harder thanks to the ``RevisionTree.find()`` method we wrote previously. Although it does require us to perform some parsing. Our API allows passing in revisions in string format, but ``find()`` only accepts a tuple of a revision number and a revision hash.
...
Ok, by now we can read and write documents. But the replication process needs two more endpoints. Luckily, they are simpler to add.

First, as previously discussed, the replication process needs to have a record of all the changes that were made to the database. This record is the reason why we keep the 'by sequence' index, which we expose to the user through the [changes API](https://docs.couchdb.org/en/stable/api/database/changes.html). This API has many options, but we limit it to the bare minimum used by replication. That includes implementing the endpoint such that it gives us all leaf revs (the non-default ``style='all_docs'`` option).
...
are set operations, it doesn't matter if revisions occur more than once on
either of the lists.
...
A **replicator** is an implementation of the [replication protocol](https://docs.couchdb.org/en/stable/replication/protocol.html). This is a process that relies only on the user-facing APIs we just designed. In fact, you could wrap the methods we just wrote up into an HTTP api, and CouchDB's replicator would be able to sync databases to and from it. So would PouchDB's replicator. This is something I did in fact do while working on this project, because it's a great way to test it. Which brings me to...
...
Finally, while writing my own replicator, I noticed that it would also be handy if that replicator could receive changes from a remote (HTTP) CouchDB, without having to expose its own HTTP API. That way, I could test my own replicator instead of CouchDB's. My solution was to wrap the CouchDB HTTP API into the same API as we just wrote. There's one problem though: HTTP requests can be slow. There are two possible solutions. The first is the most mature one: just throw a thread pool at the problem. But I thought this would be a good moment for me to try out Python's comparatively recent [``asyncio`` ecosystem](https://docs.python.org/3/library/asyncio.html) instead. I'd been wanting to do so for some time, anyway.

## Async database

The first step was to define an asynchronous API for the database. Just to be clear: this is lunacy for an in-memory database on its own. By definition, such a database is synchronous. But it makes some sense when wrapping the CouchDB HTTP API. After all, the CouchDB database in question could be hosted at the other side of the planet, introducing some real latency to the process.
...
Ah, I forgot to describe two things. First of all, there is a new method ``ensure_full_commit``. It's there because the replication protocol requires it, but for an in-memory implementation, it's a no-op. You might also wonder what ``as_future_result`` does. It's a way of making synchronous properties awaitable. The implementation is as follows:
...
- the ``source`` database, changes from here are moved to...
- the ``target`` database
...
Bidirectional replication, which is also known as sychronization, is nothing more than two unidirectional replication jobs running simultaneously with exchanged 'source' and 'target' roles.

We define the replicator as a single function. This function is long. As such, we will discuss it in multiple parts, sometimes interleaved with the functions it calls itself. While reading, I recommend having the [replication protocol documentation](https://docs.couchdb.org/en/stable/replication/protocol.html) open. It gives examples for each part of the process, and the code comments refer to its sections.
...
A good start. We store some information about how the replication process goes in ``hist_entry``. Note that ``timestamp`` is implemented as follows:
...
async def get_target_seq(target, create_target):
    try:
        ...
        raise
```
Ok, so by now both databases exist. We first want to know if replication has already occured previously between these databases by generating a replication ID:
...
Next, we query both databases for local documents named after this replication id, and we compare them. If previous replication occurred, we can start at the ``update_seq`` that was at the time the last one (stored in ``startup_checkpoint``):
...
``compare_replication_logs`` checks whether the retrieved logs are valid, and then goes through them to check if they agree on a past replication. If so, the accompanying ``recorded_seq`` is returned.
...
At this point, we either need to get all the changes from the source database or the changes since the last **checkpoint**. The checkpoint is the update sequence we just retrieved (if any). Note that the changes() function is an async generator function. That means it only generates changes when requested. For a while, no changes will be retrieved yet. We'll see that applies to a lot more functions.
...
``revs_diff_input`` converts ``Change``s into ids and revisions as expected by ``_revs_diff`` on the target database. It also keeps our history entry up-to-date. Note that this is another async generator function.
...
... before performing the actual write action. Note that this is the point at which all the generator functions start running. So this is the point at which the first change is retrieved, analysed by the revisions difference reporter, then read from disk and finally written. All in a single stream. This isn't actually strictly according to the specification, which requires batching as it was written with an HTTP API in mind. For in-memory databases this is pretty much the most efficient way of doing things, though. The HTTP implementation of the database API could work around this by performing batching internally, but that isn't fully implemented.
...
It's also worth mentioning that when performing continuous replication, none of the code below is currently run, because the above loop stays blocked waiting for new changes. Ideally, it would write a checkpoint every few changes instead. But this will do for demonstration purposes.

We're done writing. The following ensures all the writes we did are saved to disk, at least for 'normal' CouchDB installations:
...
One thing left to officially complete the replication process: merging the history entry with previous replication logs and storing updating these logs in the local documents as a new checkpoint for future replicator calls. We do that as follows:
...
Build history simply performs a merge, throwing away very old replication statistics to prevent the log from growing too large:
...
Finally, you might wonder what ``async_iter`` and ``to_list`` are doing. They are small helper functions that allow using the API without writing async loops and without having async input:
...
Finally, we also return the replication statistics to the caller:
...
While the above is a full implementation of a CouchDB-compatible database and replication, you might be interested in the HTTP API and CouchDB database wrapper I described in the testing section. The former is implemented using the [Starlette web framework](https://www.starlette.io/). The latter uses the excellent [HTTPX library](https://www.python-httpx.org/). Both can be found in my [ChairDB repository](TODO). It also includes a database implementation on top of SQLite, in case you think an in-memory database doesn't cut it. Finally, it contains some more tests. I'm not yet certain if I will continue to work on ChairDB. Perhaps.

# Conclusion

It surprised me that replication itself is actually a relatively simple process once you have all the endpoints in place. When I started out, I also expected finding the right indices to be the most complex part of this experiment. Clearly, I underestimated the complexity of working with the revision tree. Most of my time was actually spend on getting that correct (hopefully!).

If you read this far, **congratulations**! You now know how to write a CouchDB-compatible database, including replication. If you write one of your own, or if you have any remarks, comments or questions, I'd love to hear from you. [Send me an email](mailto:m@rtendevri.es), or [comment on HN](TODO).
